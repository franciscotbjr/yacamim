 ...
 *
 * @author yacamim.org.br (Manoel Veríssimo dos Santos Neto)
 * @version 1.0
 * @since 1.0
 */
public class UpdateEmployeeActivity extends YBaseListActivity {
	
	private static final String TAG = UpdateEmployeeActivity.class.getSimpleName();
	...
	protected void init() {
		try {
			...
			final Employee employee = defaultDBAdapter.getByID(1);
			Date now = new Date();
			employee.getAddress().setCity("Brasilia " + now.toString());
			employee.getProfile().setName("Profile 1 " + now.toString());
			employee.setName("Employee 1 " + now.toString());

			defaultDBAdapter.update(employee);
	...
	protected AdapterConfig buildAdapterConfig() {
		...
		final RowConfig rowConfig = new RowConfig()
			...
			.addRowConfigItem(new RowConfigItem("address.city", R.id.txtv_city_name))
			;
