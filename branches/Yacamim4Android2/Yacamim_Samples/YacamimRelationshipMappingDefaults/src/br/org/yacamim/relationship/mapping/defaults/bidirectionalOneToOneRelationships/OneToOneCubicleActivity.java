 ...
 * @since 1.0
 */
public class OneToOneCubicleActivity extends YBaseListActivity {
	
	private static final String TAG = OneToOneCubicleActivity.class.getSimpleName();
	...
	protected void init() {
		try {
			final DefaultDBAdapter<Cubicle> defaultDBAdapter = new DefaultDBAdapter<Cubicle>(Cubicle.class);
			defaultDBAdapter.open();
			List<Cubicle> cubicles = defaultDBAdapter.list();
			defaultDBAdapter.close();
			
			if(cubicles != null) {
				for(final Cubicle cubicle : cubicles) {
					Log.i(TAG, cubicle.getName() + " : " + cubicle.getResidentEmployee().getName());
				}
			}
			
			this.initList(cubicles);
	 ...
	 * @param employees
	 */
    private void initList(final List<Cubicle> cubicles) {
    	try {
			List<HashMap<String, Object>> listOfMappedData = YUtilListView.buildListOfMappedData(cubicles);
	...
	protected AdapterConfig buildAdapterConfig() {
		...
		final RowConfig rowConfig = new RowConfig()
			...
			.addRowConfigItem(new RowConfigItem("residentEmployee.name", R.id.txtv_employee_name))
			.addRowConfigItem(new RowConfigItem("name", R.id.txtv_cubicle_name))
			;
